...
class HybridKVCacheCoordinator(KVCacheCoordinator):
    ...
    def verify_and_split_kv_cache_groups(self) -> None:
        ...
        # Models such as RWKV7 expose several state tensors as distinct KV
        # cache groups that share one spec/manager, coalescing into a single
        # effective attention group. That group is self-consistent and must
        # still flow through the generic path below for EAGLE propagation, so
        # we allow a single group where upstream asserts at least two.
