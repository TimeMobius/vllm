    ...
    def parse_delta(
        ...
        *,
        ...
        if not state.prompt_reasoning_checked and prompt_token_ids is not None:
            ...
            thinking_enabled = reasoning_parser is not None and getattr(
                reasoning_parser, "thinking_enabled", False
            )
            if reasoning_parser is None or (
                self.is_reasoning_end(prompt_token_ids) and not thinking_enabled
            ):
        ...
        if self._in_reasoning_phase(state):
            ...
            elif getattr(reasoning_parser, "reasoning_markers_require_text", False):
                end_token = getattr(reasoning_parser, "end_token", None)
                should_transition = end_token is not None and end_token in current_text
            else:
            ...
            if should_transition:
                ...
                content_token_ids = (
                    delta_token_ids if self._engine_based else current_token_ids
                )
                current_token_ids = self.extract_content_ids(content_token_ids)
        ...
        if self._in_tool_call_phase(state):
            ...
            if (
                reasoning_transitioned
                and delta_message
                and delta_message.content is not None
                and not delta_message.content.strip()
            ):
                delta_message.content = None
