...
class RWKVReasoningParser(ReasoningParser):
    ...
    The RWKV vocabulary may tokenize ``<think>`` and ``</think>`` into
    multiple token IDs, so this parser tracks marker boundaries as token-id
    sequences instead of looking them up as single vocabulary entries.
    """
