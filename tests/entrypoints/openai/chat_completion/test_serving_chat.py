...
def _build_rwkv_serving_chat(
    *,
    ...
    models = OpenAIServingModels(engine, BASE_MODEL_PATHS)
    online_renderer = _build_online_renderer(
        engine,
        models.registry,
        default_chat_template_kwargs=default_chat_template_kwargs,
    )
...
def test_serving_chat_uses_generation_config_chat_template_kwargs():
    ...
    assert online_renderer.default_chat_template_kwargs == {
        "enable_thinking": True,
        "cli_default": "server",
        "model_default": "generation_config",
    }
    ...
    request_kwargs = serving_chat._effective_chat_template_kwargs(request)
...
async def test_serving_chat_does_not_add_rwkv_bad_words_for_no_thinking():
    ...
    async def fake_render_chat_request(request):
        return (
            ...
            [TokensPrompt(prompt="hi", prompt_token_ids=[1, 2])],
        )
    ...
    serving_chat.render_chat_request = fake_render_chat_request
    serving_chat.chat_completion_full_generator = fake_full_generator
    serving_chat.engine_client.generate = MagicMock(return_value=empty_generator())
...
async def _run_rwkv_streaming_answer(
    *,
    ...
    serving_chat = _build_rwkv_serving_chat(reasoning_parser="rwkv")
