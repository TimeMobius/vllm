...
def test_rwkv7_fused_recurrent_checkpoint_direct_write_matches_reference():
    if not torch.cuda.is_available():
        pytest.skip("CUDA is required to exercise RWKV7 checkpoint emission.")
    ...
    checkpoint_positions = torch.tensor([7, 15], device=device, dtype=torch.long)
    checkpoint_offsets = torch.tensor([0, 2], device=device, dtype=torch.long)
    checkpoint_slot_ids = torch.tensor([3, 6], device=device, dtype=torch.long)
    checkpoint_cache = torch.zeros(
        8,
        num_heads,
        head_dim,
        head_v_dim,
    ...
    _, _, checkpoint_ref = rwkv7_recurrent_reference_with_checkpoints(
        r=r,
        w=w,
        k=k,
        v=v,
    ...
    out_fused, state_fused, checkpoint_fused = (
        fused_mul_recurrent_rwkv7_with_checkpoints(
            ...
            initial_state=initial_state,
            output_final_state=True,
            checkpoint_state_cache=checkpoint_cache,
            checkpoint_slot_ids=checkpoint_slot_ids,
        )
    )

    assert checkpoint_fused is None
    assert out_fused.shape == v.shape
    assert state_fused is not None
    torch.testing.assert_close(
        checkpoint_cache.index_select(0, checkpoint_slot_ids),
        checkpoint_ref,
        rtol=2e-3,
        atol=1e-1,
    )
