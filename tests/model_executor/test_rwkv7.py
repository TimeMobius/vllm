...
def _make_multi_prefill_metadata(
    ...
    *,
    ...
    if len(query_lens) != len(total_seq_lens) or len(query_lens) != len(state_indices):
        raise ValueError("Prefill metadata lists must have the same length.")
...
def _make_mixed_decode_prefill_metadata(
    decode_total_seq_lens: list[int],
    prefill_query_lens: list[int],
    prefill_total_seq_lens: list[int],
    state_indices: list[int],
    *,
    device: torch.device,
) -> LinearAttentionMetadata:
    num_decodes = len(decode_total_seq_lens)
    query_start_loc = list(range(num_decodes + 1))
    running = query_start_loc[-1]
    for query_len in prefill_query_lens:
        running += query_len
        query_start_loc.append(running)

    seq_lens = decode_total_seq_lens + prefill_total_seq_lens
    return LinearAttentionMetadata(
...
def test_rwkv7_block_batches_prefill_tokens_without_changing_results():
    ...
    with set_current_vllm_config(vllm_config):
        ...
        try:
            block_batched = RWKV7Block(
                config=config, layer_idx=1, prefix="model.layers.0"
            )
            _initialize_module_parameters(block_batched)
            block_ref = RWKV7Block(config=config, layer_idx=1, prefix="model.layers.1")
            block_ref.load_state_dict(block_batched.state_dict())
            ...
            output_batched, v_first_batched = block_batched(
                hidden_states, v_first.clone(), metadata
            )

            output_ref = torch.empty_like(hidden_states)
            v_first_ref = torch.empty_like(v_first)
            start = 0
            for slot_id, query_len, total_seq_len in zip(
                [0, 1], query_lens, total_seq_lens
            ):
                end = start + query_len
                states = block_ref._get_kv_state(
                    slot_id, use_initial_state=(total_seq_len > query_len)
                )
                out, v_first_out, attn_shift, recurrent, ffn_shift = block_ref._run_sequence(
...
def test_rwkv7_block_handles_mixed_decode_and_prefill_batches():
    config = _make_config()
    vllm_config = VllmConfig(device_config=DeviceConfig("cpu"))
    with set_current_vllm_config(vllm_config):
        init_distributed_environment(
        ...
        try:
            ...
            metadata = _make_mixed_decode_prefill_metadata(
                decode_total_seq_lens=[8],
                prefill_query_lens=[3, 2],
                prefill_total_seq_lens=[3, 7],
                state_indices=[0, 1, 2],
                device=torch.device("cpu"),
            )
            hidden_states = torch.randn(
            ...
            output_batched, v_first_batched = block_batched(hidden_states, None, metadata)

            output_ref = torch.empty_like(hidden_states)
            v_first_ref = torch.empty_like(hidden_states)

            decode_out, decode_v_first, attn_shift, recurrent, ffn_shift = (
                block_ref._run_sequence(
                    hidden_states[:1],
                    None,
                    *block_ref._get_kv_state(0, use_initial_state=True),
                )
            )
            output_ref[:1] = decode_out
            v_first_ref[:1] = decode_v_first
            block_ref._store_kv_state(0, attn_shift, recurrent, ffn_shift)

            prefill_specs = [(1, 4, 1, False), (4, 6, 2, True)]
            for start, end, slot_id, use_initial_state in prefill_specs:
                out, v_first_out, attn_shift, recurrent, ffn_shift = block_ref._run_sequence(
                    hidden_states[start:end],
                    None,
                    *block_ref._get_kv_state(slot_id, use_initial_state=use_initial_state),
                )
                output_ref[start:end] = out
                v_first_ref[start:end] = v_first_out
                block_ref._store_kv_state(slot_id, attn_shift, recurrent, ffn_shift)
